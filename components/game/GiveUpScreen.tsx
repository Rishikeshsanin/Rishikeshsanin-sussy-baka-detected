  ...
  return (
    <section className="screen-enter glass-card w-full max-w-2xl rounded-[2rem] px-5 py-8 text-center sm:px-10 sm:py-10">
      ...
      <span className="mx-auto inline-flex items-center gap-2 rounded-full border border-white/8 bg-white/[0.03] px-3 py-1.5 text-[0.62rem] font-semibold uppercase tracking-[0.19em] text-white/44">
        <Crown size={13} /> Your victory
      </span>
      <h1 className="mt-5 text-4xl font-semibold tracking-[-0.055em] text-white sm:text-5xl">You beat the oracle.</h1>
      <p className="mx-auto mt-4 max-w-md text-sm leading-6 text-white/44">
        {message ?? "You found a corner of the character universe I couldn’t reach this time."}
      </p>

      <div className="soft-divider my-7" />

      {submittedName ? (
        <div className="rounded-2xl border border-teal-200/10 bg-teal-200/[0.04] px-5 py-4 text-sm text-teal-50/66">
          <span className="font-semibold text-white">{submittedName}</span> was saved to this game on your device. No global learning claimed.
        </div>
      ) : (
        <form
        ...
        >
          <label htmlFor="secret-character" className="mb-2 block text-xs font-medium text-white/52">
            Who were you thinking of? <span className="text-white/26">(optional)</span>
          </label>
